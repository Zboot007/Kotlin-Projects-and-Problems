fun encode(s: String)
    = Regex("([a-z])\\1*").replace(s) { "${it.value[0]}${it.value.length}" }
fun testEncode() {
    val s1 = "aeeeffrgtt"
    val s2 = "abcdef"
...
fun decode(s: String)
    = Regex("([a-z])(\\d+)").replace(s) { it.groupValues[1].repeat(it.groupValues[2].toInt()) }
fun testDecode() {
    val s1 = "a1e3f2r1g1t2"
    val s2 = "a1b1c1d1e1f1"
...
fun main() {
    ...
    // ====> basic methods <====
    // component1(): E | componentN(): E  N from 1 to 5
    ...
    // addAll(Iterable<E> | Collection<E> | Sequence<E> | Array<E>): Boolean | +=
...
//    println(matrix)
//    println(matrix.flatten().map { it * 10 })
//    println(matrix.flatMap { it.map { it * 10 } })
    // flatten(): List<E>
...
//    val names = listOf("Mike", "Tom", "Anna", "Nicole", "John", "Sam")
...
//    ) { it.uppercase() })
    ...
    // [a, b, c].zip([1, 2, 3]) { e, r -> "$e$r" } -> ["a1", "b2", "c3"]

    // [a, b, c, d].zipWithNext() -> [[a, b], [b, c], [c, d]]
    // [a, b, c, d].zipWithNext() { p, c -> "$p$c" } -> ["ab", "bc", "cd"]

    // [[a, 1], [b, 2], [c, 3]].unZip() -> [[a, b, c], [1, 2, 3]]
